...
export function GameHUD() {
  ...
  return (
    <Card className="w-full md:w-64 bg-slate-900 text-white p-4 rounded-lg flex flex-col gap-4 border border-slate-700 shadow-lg">
      <h2 className="text-xl font-bold border-b border-slate-700 pb-2">🎮 Game Info</h2>
      <PhaseSection />
      <TurnSection />
        ...
        <div className="bg-slate-800 p-3 rounded border border-slate-700 space-y-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-300">
              Hits: <span className="font-bold">{playerHits}</span> Misses: <span className="font-bold">{playerMisses}</span>
            </p>
            <p className="text-xs text-slate-400">Ships: {totalPlayerShips}</p>
          </div>
          {/* AI Board */}
          <div>
            <p className="text-sm font-semibold text-orange-400">AI Board</p>
            <p className="text-xs text-slate-300">
              Hits: <span className="font-bold">{aiHits}</span> • Misses: <span className="font-bold">{aiMisses}</span>
            </p>
            <p className="text-xs text-slate-400">Ships: {totalAiShips}</p>
          </div>
        </div>
      </div>
      ...
      {phase === GamePhase.PLACEMENT && (
        <div className="flex flex-col gap-2 text-sm">
          <div className="flex items-center gap-2">
            <Badge className={playerReady ? "text-green-400" : "text-yellow-400"}>
              {playerReady ? "✓" : "○"}
            </Badge>
      ...
      {/* Start Battle Button */}
      {phase === GamePhase.PLACEMENT && (
        <Button variant="primary" disabled={!playerReady || !aiReady} className="mt-auto">
